...
public class PlayersMove : MonoBehaviour
{
    ...
    void Start ()
    {
        ...
        //Vector3 pos = posList[firstNum];
        //pos.y = transform.position.y;
        //transform.position = pos;
        //targetNum = firstNum + 1;
        //nowDirNum = firstNum;
    ...
    /* @brief   フックショット*/
    void HookShot(Vector3 nowPos, Vector3 targetPos)
    {
        // 重力オフ
        partnerRig.velocity = Vector3.zero;

        // 目標へ移動
        nowPos = Vector3.MoveTowards(nowPos, targetPos, Time.deltaTime * rollUpPower);
    }
    ...
    void ChildOnTriggerStay(Collider col)
    {
        if (state == State.eNormal)
        {
            ...
            {
                playerPath.SetInput(partnerPos.transform.position.x - transform.position.x, 1);
                HookShot(partnerPos.transform.position, transform.position);
            }
